    ...
    if (isset($keyword)) {
        ...
        if ($sradio == "medicine") {
            if ($keyword == '') {
                $query = "SELECT m.M_ID as `#ID`, m.Name as `Medicine`, c.Name as `Company`, m.Price
                        ...
                        WHERE m.M_ID = mf.M_ID AND mf.C_ID = c.C_ID ORDER BY m.M_ID LIMIT 25;";
            }
            else {
                $query = "SELECT m.M_ID as `#ID`, m.Name as `Medicine`, c.Name as `Company`, m.Price
                        FROM medicine m, company c, manufacturer mf
                        WHERE m.M_ID = mf.M_ID AND mf.C_ID = c.C_ID AND m.Name LIKE '%$keyword%' ORDER BY m.M_ID LIMIT 25;";
            }
        ...
        elseif ($sradio == "pharm") {
            ...
            if (mysqli_num_rows($result) == 0) {
                ...
                echo" <table border='1' class='greenTable table-hover'>
                  <thead>
                      <tr>
                          <th>#ID</th>
                          <th>Email</th>
